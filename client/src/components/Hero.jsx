...
const Hero = () => {
  const { axios, searchedCities, setSearchedCities } = useAppContext()
  ...
  const onSearch = async (e) => {
    ...
    try {
      await axios.post('/api/user/recent-searches', { city: destination })
    } catch (error) {
      console.log("Search save error:", error.message)
    }
    ...
    setSearchedCities((prevSearchedCities) => {
      const updatedSearchedCities = [
        destination,
        ...prevSearchedCities.filter(c => c !== destination)
      ]
  ...
  return (
    ...
    >
      ...
      <h1 className='font-playfair text-2xl md:text-[56px] md:leading-[56px]
